...
import React, { useState, useEffect, useRef } from 'react';
import { Moon, Sun, Github, Twitter, Linkedin, Mail, Code2, Award, Briefcase, GraduationCap, User, ExternalLink, Database, Server, Layers, Download, Music, Play, Pause } from 'lucide-react';
import * as THREE from 'three';
import { gsap } from 'gsap';
...
const Portfolio = () => {
  ...
  // Music button continuous animation when playing
 useEffect(() => {
    if (musicButtonRef.current) {
      ...
      if (isPlaying) {
        ...
        gsap.to(musicButtonRef.current, {
          rotation: 0, // Ensure icon resets to straight orientation
          duration: 0.5,
          ease: "power2.out"
        });
      }
    ...
    // Cleanup: stop animations and reset rotation
    return () => {
      if (musicButtonRef.current) {
        gsap.killTweensOf(musicButtonRef.current);
        gsap.to(musicButtonRef.current, {
...
useEffect(() => {
  ...
  const bgClass = isDark ? 'bg-black' : 'bg-blue-50/90';
  const textClass = isDark ? 'text-white' : 'text-gray-900';
  const cardBg = isDark ? 'bg-gray-900/80' : 'bg-gray-50/80'; // Increased opacity for better visibility
